...
class TreeDragDropMixin:
    def dropMimeData(self, parent, index, data, action):
        ...
        if parent is None and selected_items:
            dragged_id = selected_items[0].data(0, Qt.UserRole)
    ...
    def dropEvent(self, event):
        ...
        if drop_pos == QTreeWidget.OnItem:
            self.request_move.emit(dragged_id, target_id, -1)
            event.accept()
            return
        else:
            ...
            if parent_item is None and (not self.model.root or dragged_id != self.model.root.id):
                event.ignore()
                return
            parent_id = parent_item.data(0, Qt.UserRole) if parent_item else self.model.root.id
            ...
            if drop_pos == QTreeWidget.BelowItem:
                target_index += 1
            self.request_move.emit(dragged_id, parent_id, target_index)
            event.accept()
            return
